...
import NeurealReward from 'Embark/contracts/NeurealReward';
  ...
  EmbarkJS.onReady(async (err) => {
    ...
    if (web3.utils.isAddress(contractAddr)) {
      curContract = new EmbarkJS.Blockchain.Contract({
        abi: NeurealReward.options.jsonInterface,
      ...
      $('#div_deployContract #button_deploy').click(async function () {
        curContract = await NeurealReward.deploy({ data: NeurealReward.options.data }).send({ gas: 6000000 });
